...
| Native profile persistence seam | Title/CRC-gated profile update of the native JSON container at `CProfile::SaveGame` completion and restore before `CProfile::LoadGame`; game slots still use the card path | `thirdparty/pcsx2/pcsx2/AVPE/NativeSaveBackend.*`, composed by `NativeEeExecutionHooks.*` | `AVPE::NativeSaveBackend::{ObserveProfileEntry,ObserveProfileReturn,ObserveProfileLoadEntry}` | [save-path RE contract](re/save-path.md) |
| Native save path | Platform user-data resolution for the AVPE native container; no checkout or scratch fallback | `src/avpe/native_save_path.py` | `resolve_native_save_path()` | [save-path RE contract](re/save-path.md) |
