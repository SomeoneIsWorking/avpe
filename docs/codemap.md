...
| Normal input-dispatch observer | Bounded `GInputDevice` callback identities, owner-vtable evidence, and exact queued pointer/menu callback replacement at the shipping dispatch boundary | `thirdparty/pcsx2/pcsx2/AVPE/NativeInputDispatch.cpp/.h` | `AVPE::NativeInputDispatch` | [input-path contract](re/input-path.md) |
