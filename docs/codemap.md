...
| SaveEx payload evidence parser | Bounded readers for selected fixed, bitmap, message-queue, and conditional class payloads; no recursive stream ownership | `src/avpe/save_ex.py` | `parse_gunit_payload()`, `parse_gobject_ai_payload()` | [save-path RE contract](re/save-path.md) |
...
| BIOS/IOP observation census | Bounded sequence-ordered observation of IOP imports, module registration, interrupt registration, and SIF RPCs; no service behavior ownership | `thirdparty/pcsx2/pcsx2/AVPE/NativeBiosTrace.*`, narrow calls in `IopBios.cpp` | `AVPE::NativeBiosTrace::SnapshotJson()` | [BIOS/IOP contract](re/bios.md) |
...
├── NativeBiosTrace.*          bounded BIOS/IOP observation census
