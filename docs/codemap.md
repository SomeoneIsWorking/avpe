...
| BIOS/IOP census event store | Bounded sequence-ordered event admission, identity coalescing, JSON serialization, and exact EE BIOS entry/return pairing under the trace owner's lock | `thirdparty/pcsx2/pcsx2/AVPE/NativeBiosEventStore.*` | `AVPE::NativeBiosEventStore::Store` | [BIOS/IOP contract](re/bios.md) |
...
├── NativeBiosEventStore.*     bounded BIOS/IOP events and EE BIOS return pairing
