...
| BIOS runtime-event interpretation | Shared occurrence-count validation, typed exception/counter field validation, and joint-identity summaries retaining first counter samples; no IRQ scheduling or handler inference | `src/avpe/bios_runtime_events.py` | `event_occurrences()`, `runtime_event_is_verified()`, `summarize_runtime_events()` | [BIOS/IOP contract](re/bios.md) |
