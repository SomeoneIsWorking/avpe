...
| Native save container store | Versioned title-bound host container, parser-backed slot validation, integrity digests, and atomic replacement; no game-boundary interception yet | `src/avpe/native_save_store.py` | `write_slot()`, `read_slot()`, `list_slots()` | [save-path RE contract](re/save-path.md) |
