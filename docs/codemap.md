...
|---|---|---|---|---|
| Launcher shim | Stable locked-environment product entry | `run.sh` | `uv run --frozen avpe launch` | — |
...
| Private BIOS diagnostic phase dispatch | Routes bounded semaphore, event-flag, and thread diagnostics without absorbing their service-specific policies | `src/avpe/native_bios_diagnostic_phases.py` | `run_diagnostic_phase()` | [BIOS/IOP contract](re/bios.md) |
...
| BIOS runtime-event interpretation | Shared occurrence-count validation, typed exception-transition/counter field validation, and joint-identity summaries retaining first counter samples; no IRQ scheduling or handler inference | `src/avpe/bios_runtime_events.py` | `event_occurrences()`, `runtime_event_is_verified()`, `summarize_runtime_events()` | [BIOS/IOP contract](re/bios.md) |
...
| Authored archive inspection | Bounded RIFF/TBF traversal, exact compressed-chunk framing, and decoded byte searches with chunk provenance; reuses the save-format BWJ decoder, no runtime reachability inference | `src/avpe/tbf_archive.py`, `tools/search_tbf.py` | `search_archive()` | [disc-I/O RE contract](re/disc-io.md) |
...
| Native save container store | Versioned title-bound host container for the grounded profile payload and parser-backed game slots, with integrity digests and atomic replacement; provisioning/import owner | `src/avpe/native_save_store.py` | `write_profile()`, `read_profile()`, `write_slot()`, `read_slot()`, `list_slots()`, `replace_container()` | [save-path RE contract](re/save-path.md) |
| Native profile contract | Validated live `CProfile` object/payload capture and bounded guest payload restoration at the grounded profile layout | `thirdparty/pcsx2/pcsx2/AVPE/NativeProfileContract.*` | `AVPE::NativeProfileContract::{CaptureCurrent,RestorePayload}` | [save-path RE contract](re/save-path.md) |
| Native profile persistence seam | Title/CRC-gated profile update under AVPE's configured persistent datapath at `CProfile::SaveGame` completion and restore before `CProfile::LoadGame`; game slots still use the card path | `thirdparty/pcsx2/pcsx2/AVPE/NativeSaveBackend.*`, composed by `NativeEeExecutionHooks.*` | `AVPE::NativeSaveBackend::{ObserveProfileEntry,ObserveProfileReturn,ObserveProfileLoadEntry}` | [save-path RE contract](re/save-path.md) |
| Native save path | Platform user-data root shared by product launch and import; container lives directly under that root, with no checkout or scratch fallback | `src/avpe/native_save_path.py` | `resolve_native_data_root()`, `resolve_native_save_path()` | [save-path RE contract](re/save-path.md) |
| Native memory-card importer | Bounded PS2 raw-card superblock, indirect-FAT, directory-chain, profile-record, and game-slot validation feeding an atomic native container replacement; no live card interception | `src/avpe/memory_card_import.py` | `import_memory_card()` | [save-path RE contract](re/save-path.md) |
| Native profile save probe | Validation policy for the bounded live `CProfile` payload snapshot captured at the exact game-save entry | `src/avpe/native_save_probe.py`; composed by `src/avpe/native_bios_probe.py` | `profile_snapshot_is_verified()` | [save-path RE contract](re/save-path.md) |
...
| EE-call execution | Deferred admission without register mutation, context installation at the outer VM executor boundary, return-PC stop, bounded synchronous calls, and result handling | `thirdparty/pcsx2/pcsx2/AVPE/EECallShuttle.cpp/.h`; executor composition in `VMManager.cpp` | `AVPE::EECallShuttle::{BeginVmExecution,EndVmExecution,RunTransaction}` | [input-path contract](re/input-path.md) |
...
| Native menu control route | Typed menu/movie endpoint dispatch, diagnostic parsing, failure/status mapping, and JSON presentation for state/actions and readiness requests | `thirdparty/pcsx2/pcsx2/AVPE/NativeMenuRoute.cpp/.h` | `AVPE::NativeMenuRoute::Handle()` | [control-channel contract](re/control-channel.md) |
...
| Live prompt-font observation | Bounded, read-only capture of the exact `CzFont::Render` font, glyph map, attached text resource, and text bytes; EE hooks compose it, and the route only presents the diagnostic snapshot | `thirdparty/pcsx2/pcsx2/AVPE/NativePromptTrace.*`, `NativePromptRoute.*` | `AVPE::NativePromptTrace::Observe()`, `AVPE::NativePromptRoute::Handle()` | [prompt producer investigation](issues/0008-replace-ps2-button-prompts-with-pc-native-prompt.md) |
| Live mesh screen-bounds observation | Bounded, read-only capture of `CMeshWorkspace::GetMatrix`'s live screen-space AABB output (mesh/render/bounds-object identity plus xmax/ymax/xmin/ymin) at its exact post-call return PC; EE hooks compose it, and the route only presents the diagnostic snapshot | `thirdparty/pcsx2/pcsx2/AVPE/NativeMeshBoundsTrace.*`, `NativeMeshBoundsRoute.*` | `AVPE::NativeMeshBoundsTrace::Observe()`, `AVPE::NativeMeshBoundsRoute::Handle()` | [prompt producer investigation](issues/0008-replace-ps2-button-prompts-with-pc-native-prompt.md) |
| Synchronous mesh-bounds capture probe | Composes the pause-admission proof with the mesh-bounds trace arm/poll/capture/stop sequence entirely inside one control-test process, avoiding cross-turn HTTP timing dependence on a still-live VM | `src/avpe/native_mesh_bounds_probe.py` | `probe_native_mesh_bounds()` via `tools/run_control_test.py --probe-native-mesh-bounds` | [prompt producer investigation](issues/0008-replace-ps2-button-prompts-with-pc-native-prompt.md) |
| Frame snapshot route | Current GS frame capture and bounded bottom-up BGR BMP encoding for the diagnostic `/snap` response | `thirdparty/pcsx2/pcsx2/AVPE/NativeSnapshotRoute.*` | `AVPE::NativeSnapshotRoute::Handle()`, `EncodeBmp()` | [control-channel contract](re/control-channel.md) |
...
| Diagnostic semaphore phase | Private nonblocking EE semaphore lifecycle, signed result and stack-restoration admission, and v7 census capture; no title semaphore ownership or HLE implementation | `src/avpe/native_semaphore_probe.py` | `probe_semaphore_lifecycle()` via `tools/run_control_test.py --probe-bios-phase semaphore` | [BIOS/IOP contract](re/bios.md) |
| Diagnostic event-flag phase | Grounded non-waiting event-flag wrapper result vector and stack-restoration admission; no wait/clear semantics or HLE implementation | `src/avpe/native_event_flag_probe.py` | `probe_event_flag_invalid_id()` via `tools/run_control_test.py --probe-bios-phase event-flag-negative` | [BIOS/IOP contract](re/bios.md) |
| Diagnostic interrupt-handler phase | Captures grounded interrupt-controller wrapper result shapes for an all-ones handler token; no handler delivery or HLE implementation | `src/avpe/native_interrupt_handler_probe.py` | `probe_interrupt_handler_invalid_id()` via `tools/run_control_test.py --probe-bios-phase interrupt-handler-negative` | [BIOS/IOP contract](re/bios.md) |
| Diagnostic SIF-register phase | Captures grounded `sceSifGetReg` values for the five low selectors from a restored state; no SIF state mutation or HLE implementation | `src/avpe/native_sif_query_probe.py` | `probe_sif_registers()` via `tools/run_control_test.py --probe-bios-phase sif-query` | [BIOS/IOP contract](re/bios.md) |
...
| Diagnostic OSD-config v2 phase | Captures the grounded full-span `GetOsdConfigParam2` output-buffer write; no OSD v2 policy or HLE implementation | `src/avpe/native_osd_config2_probe.py` | `probe_osd_config2_output()` via `tools/run_control_test.py --probe-bios-phase osd-config2-output` | [BIOS/IOP contract](re/bios.md) |
| Diagnostic COP0 phase | Captures a stable read-only subset of `GetCop0` register results from a restored state; volatile timing and exception registers remain excluded | `src/avpe/native_cop0_probe.py` | `probe_cop0_registers()` via `tools/run_control_test.py --probe-bios-phase cop0-query` | [BIOS/IOP contract](re/bios.md) |
...
| Guest-reset BIOS diagnostic | Arms the bounded BIOS sink around the real VM reset and records module-release presence or an explicit negative; no shutdown ownership or HLE implementation | `src/avpe/native_bios_reset_probe.py`, dispatched by `src/avpe/native_bios_probe.py` | `run_guest_reset_phase()` via `tools/run_control_test.py --probe-bios-phase guest-reset` | [BIOS/IOP contract](re/bios.md) |
...
| CPU exception observation | CPU-thread scoped before/after register snapshots across actual EE/IOP exception routines, including early returns; no CPU semantics or handler execution inference | `thirdparty/pcsx2/pcsx2/AVPE/NativeExceptionObservation.*` | `AVPE::NativeExceptionObservation` | [BIOS/IOP contract](re/bios.md) |
...
├── native_bios_diagnostic_phases.py  private diagnostic phase dispatch
├── native_bios_call.py         shared typed EE-call transport for BIOS phases
├── native_semaphore_probe.py   private semaphore lifecycle proof policy
├── native_event_flag_probe.py  non-waiting event-flag result proof policy
├── native_interrupt_handler_probe.py  interrupt-handler result proof policy
├── native_sif_query_probe.py  SIF-register result proof policy
...
├── NativeExceptionObservation.* CPU-thread exception-transition snapshots
...
├── NativeMovieInput.*         one-shot movie readiness and original abort dispatch
