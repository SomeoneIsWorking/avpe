...
scratch/control-test/game-load-bios.json; scratch/control-test/game-load-bios-inventory.json; two successful 2026-09-12 surfaceless/null-muted game-load phases using the runner's savestate-matched PAL BIOS; focused native and Python probe tests. Each run reached `CProfile::LoadGame` at `0x00130000`, returned zero at `0x00130168`, completed the synchronous mission-goals modal, and left the source card unchanged. The runner now reads the savestate console identifier and replaces a stale isolated BIOS link before launch.
