...
The native observation now records the actual guest `ioman.open` result for
handled paths. A clean native run returned descriptor `256` for `TBF.TBF` and
signed `-2` (`ENOENT`) for the boot-time `23ACA1AA.TBX` and `NOMEMLOGO.TBD`
misses, each with one refusal and no dropped path. This closes the issue's
representative namespace and guest-visible failure-result acceptance; the
remaining byte-differential and timing work belongs to S023/S024.
