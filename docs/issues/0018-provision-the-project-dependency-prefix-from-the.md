...
symptom: A fresh checkout has no build/deps Qt prefix, so prepare must provision it before configuring the AVPE target
state_items: S012
...
it; `build/deps` is treated as a pre-existing developer artifact.
...
dependency workflow, runs it under the top-level `build/` root with
`BUILD_FFMPEG=0`, refuses
