...
candidate. `GMenuItem::GetText` statically resolves to offset `+0x148`; live
bounded reads establish their exact order as `Start Campaign`, `Load Game`,
`Tutorials`, `Options`, `Bestiary`, and `Extras`. The `Main_Quit` archive label
must belong to a descendant menu or another title state; activating any of
...
(`0x002B3EF0`), `GsGetIMR` (`0x002B4140`), `PSMode` (`0x002B4260`),
`MachineType` (`0x002B4270`), and `GetMemorySize` (`0x002B4280`). Two isolated
runs returned `1`, `0x01FF5000`, `0x00000080`, the u64 value
`0x000000000000FF00`, an explicitly unobserved `PSMode` result, `0x00008000`,
and `0x02000000`, respectively, with restored stacks. `GsGetIMR` uses the
BIOS trace's declared u64 disposition; its separate `v1` register is not
folded into that value. `GetGsHParam` (`0x002B3EE0`) is deliberately omitted:
the production disposition marks it as a void output-pointer service, so its
register contents are not a result. The void classification for `PSMode` is
preserved from the production trace for the same reason. This closes only
