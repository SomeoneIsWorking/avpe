...
def main() -> int:
    ...
    parser.add_argument("--probe-native-pause-menu", action="store_true",
                        help="press PAD_START from gameplay and require the resulting live menu")
    ...
    if (args.use_native_assets or args.probe_native_asset_reads
            ...
            or args.probe_native_movie_reads
            or args.probe_native_stream_reads
            or args.probe_native_marine_m1_transition or args.probe_bios_phase == "mission"
            or native_recovery_requested
            or args.probe_native_asset_reset
            or args.probe_asset_byte_trace
            ...
            if status_is_verified(status, nonce):
                ...
                if args.probe_native_mouse and probe_error is None:
                    try:
                        ...
                        probe_error = str(error)
                if args.probe_native_pause_menu and probe_error is None:
                    try:
                        pause_menu_proof = probe_gameplay_pause_menu(port, deadline)
                    except (RuntimeError, ValueError, json.JSONDecodeError) as error:
                        probe_error = str(error)
                ...
                if args.probe_native_menu_activate and probe_error is None:
                    try:
                        menu_activation_proof = probe_native_menu_activation(
                            port, deadline, LOG_DIR.parent)
                    except (RuntimeError, ValueError, json.JSONDecodeError) as error:
                        probe_error = str(error)
